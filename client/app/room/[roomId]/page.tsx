...
// // ── Floating Draggable Self-View Component (Picture-in-Picture) ──
// function DraggableSelfView({ trackRef }: { trackRef: TrackReference }) {
//   const [position, setPosition] = useState({ x: 20, y: 80 });
//   const [isDragging, setIsDragging] = useState(false);
//   const [isMinimized, setIsMinimized] = useState(false);

//   const dragRef = useRef({ startX: 0, startY: 0, initialX: 20, initialY: 80 });
...
//           : 'h-36 w-48 sm:h-40 sm:w-56 rounded-2xl'
//       } overflow-hidden border-2 border-indigo-500/80 bg-slate-950 shadow-2xl backdrop-blur-xl group`}
//     >
//       {/* Drag & Controls Overlay */}
//       <div className="absolute top-2 left-2 right-2 z-30 flex items-center justify-between">
//         <span className="cursor-grab text-[9px] font-bold text-white/90 bg-slate-900/80 px-2 py-0.5 rounded-full backdrop-blur-sm border border-white/10">
//           ✋ Drag Me
//         </span>
...
//           }}
//           className="h-5 w-5 rounded-full bg-white/20 hover:bg-white/40 text-white text-[10px] font-bold flex items-center justify-center backdrop-blur-sm cursor-pointer"
//         >
//           {isMinimized ? '⤢' : '─'}
//         </button>
//       </div>
...
//               <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-white/20 bg-white/10 text-white shadow-md">
//                 <GhostIcon className="h-5 w-5" />
//               </div>
//               <p className="mt-1 text-[10px] text-slate-400 font-medium">Camera Off</p>
//             </div>
//           )}

//           <div className="absolute bottom-2 left-2 z-20 flex items-center gap-1.5 rounded-full border border-white/20 bg-slate-900/85 px-2.5 py-1 text-[10px] font-bold text-white backdrop-blur-md">
//             <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
//             <span className="truncate max-w-[100px]">{displayName} (You)</span>
//           </div>
//         </>
//       )}
//     </div>
...
//   // If remote participants exist, show remote tracks on main screen & local user in floating box!
...
//       <div className={`grid flex-1 auto-rows-fr ${getGridClasses(mainTracks.length)} gap-3 sm:gap-4 min-h-0 h-full`}>
...
function CustomVideoConference({ onLeave }: { onLeave: () => void }) {
  ...
  return (
    <div className="relative flex h-[calc(100dvh-4.5rem)] w-full flex-col overflow-hidden bg-[#f1f3f9] p-3 pb-16">
      <div className={`grid h-full min-h-0 flex-1 auto-rows-fr ${getGridClasses(mainTracks.length)} gap-3 sm:gap-4`}>
        {mainTracks.map((trackRef) => (
          <CustomParticipantTile
            key={`${trackRef.participant.identity}_${trackRef.source}`}
            trackRef={trackRef}
